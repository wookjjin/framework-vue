...
const addColumn = () => {
  ...
  columns.value.push({
    key: newKey,
    title: `${props.newGroupName} ${columns.value.length + 1}`,
  })
}
...
const addTask = (column: KanbanColumn) => {
  ...
  const newTask: KanbanTask = {
    id: `${Date.now()}`,
    key: column.key,
    title: `${props.newTaskName} ${column.key} - ${taskCount}`,
  }
...
const onTaskDrop = () => {
  if (!draggingTask.value || !dragOverColumnKey.value) return;
...
<template>
  <div class="kanban-board-container">
    <div class="kanban-board">
      <ul v-for="(column, index) in columns" :key="`kanban-group-${column.key}`" class="kanban-column" draggable="true"
        ...
        <slot :name="`column-${column.key}`">
          ...
          <div class="kanban-empty" v-if="tasksByColumn[column.key]?.length === 0"
            @dragover.prevent="onTaskDragOver(column.key, null)" @drop="onTaskDrop">
            Empty List
          </div>
        </slot>
        <li v-for="task in tasksByColumn[column.key]" :key="task.id" class="kanban-card" draggable="true"
